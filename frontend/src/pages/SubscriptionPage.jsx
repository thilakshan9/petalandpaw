...
import { getActiveSeason, seasonConfig, HW, XMAS } from "@/lib/halloween";
...
export default function SubscriptionPage() {
  ...
  const handleSubscribe = async () => {
    ...
    const hwNote = halloweenThemed[planId] ? `${cfg.tagEmoji} ${cfg.tagLabel} themed bouquet requested. ` : "";
    try {
      ...
      if (data.url) {
        ...
        if (plan) {
          ...
          const cartItem = {
            ...
            add_pet_toy: toyOn,
            theme: halloweenThemed[planId] ? season : undefined,
          };
            ...
            {plans.map((plan, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Seasonal themed add-on (October/December only) */}
                  {seasonActive && (
                    <label
                      className="flex items-center justify-between rounded-xl px-4 py-3 mb-6 cursor-pointer border transition-colors"
                      style={{
                        borderColor: isXmas ? XMAS.greenBorder : HW.purpleBorder,
                        background: halloweenThemed[plan.id]
                          ? (isXmas ? "rgba(46,93,63,0.16)" : "rgba(107,78,113,0.16)")
                          : (isXmas ? XMAS.greenTint : HW.purpleTint),
                      }}
                      data-testid={`halloween-toggle-${plan.slug}`}
                    >
                      <span className="flex items-center gap-2">
                        <span aria-hidden="true">{cfg.emoji}</span>
                        <span className="text-sm text-[#2C2C2C]">{cfg.themeAddLabel}</span>
                      </span>
                      <input
                        type="checkbox"
                        ...
                        className="w-4 h-4"
                        style={{ accentColor: isXmas ? XMAS.green : HW.purple }}
                  ...
                  <Button
                    onClick={() => {
                      ...
                      addToCart({
                        ...
                        add_pet_toy: toyOn,
                        theme: halloweenThemed[plan.id] ? season : undefined,
                      });
